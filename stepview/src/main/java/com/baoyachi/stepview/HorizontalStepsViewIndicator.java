...
//        mCompletedPaint = mUnCompletedPaint ;
        //已经完成线的宽高 set mCompletedLineHeight
        mCompletedLineHeight = 0.05f * defaultStepIndicatorNum / 2;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < mStepNum; i++) {
            ...
            float paddingLeft = (screenWidth - mStepNum * mCircleRadius * 2 - (mStepNum - 1) * mLinePadding) / (mStepNum + 1);
            //add to list
            mCircleCenterPointPositionList.add(paddingLeft * (i + 1) + mCircleRadius + i * mCircleRadius * 2 + i * mLinePadding);
        }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
            ...
            if (i <= mComplectingPosition && mStepBeanList.get(0).getState() != StepBean.STEP_UNDO)//判断在完成之前的所有点
            {
                ...
                canvas.drawRect(preComplectedXPosition + mCircleRadius - 10 + 20, mLeftY, afterComplectedXPosition - mCircleRadius + 10 - 20, mRightY, mCompletedPaint);
            } else {
                mPath.moveTo(preComplectedXPosition + mCircleRadius + 20, mCenterY);
                mPath.lineTo(afterComplectedXPosition - mCircleRadius - 20, mCenterY);
...
//                canvas.scale(0.5f,0.5f);
                Rect rect1 = new Rect(rect.left + (rect.right - rect.left) / 5, rect.top + (rect.bottom - rect.top) / 5, rect.right - (rect.right - rect.left) / 5, rect.bottom - (rect.bottom - rect.top) / 5);
//                Rect rect1 =new Rect((int) (currentComplectedXPosition - mCircleRadius)-20, (int) (mCenterY - mCircleRadius), (int) (currentComplectedXPosition + mCircleRadius)+20, (int) (mCenterY + mCircleRadius));
                mDefaultIcon.setBounds(rect1);
                mDefaultIcon.draw(canvas);
                canvas.restore();
            } else if (stepsBean.getState() == StepBean.STEP_CURRENT) {
                mCompletedPaint.setColor(Color.WHITE);
//                canvas.drawCircle(currentComplectedXPosition, mCenterY, mCircleRadius * 0.5f, mCompletedPaint);
                Rect rect1 = new Rect(rect.left + rect.right / 2, rect.top - rect.right / 2, rect.right / 2, rect.bottom / 2);
                mAttentionIcon.setBounds(rect1);
