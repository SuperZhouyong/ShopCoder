...
public class PullZoomView extends ScrollView {
    ...
    private int contentHright ;             //解决weight属性不起作用的
    ...
    private int maxY = 100;                       //允许的最大滑出距离
    private int touchSlop;

    private OnScrollListener scrollListener;  //滚动的监听
    private MarginLayoutParams Contentpar;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Contentpar = (MarginLayoutParams) contentView.getLayoutParams();
        contentHright = contentView.getMeasuredHeight() ;
...
//                        LogUtils.logd("测试ContentHeight--"+"contentHeight"+contentHright+"----"+contentView.getLayoutParams().height+"-----"+height+"原始头部高度"+headerHeight);
                        Contentpar.height = contentHright ;
                        contentView.setLayoutParams(Contentpar);
