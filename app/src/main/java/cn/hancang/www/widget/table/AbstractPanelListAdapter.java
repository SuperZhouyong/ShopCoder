...
public abstract class AbstractPanelListAdapter {
    ...
    private java.lang.String TAG = "AbstractPanelListAdapter";
...
//        ll_row.setBackgroundColor(context.getResources().getColor(R.color.black));
        ...
        ll_row.setPadding(0, 0, (int) context.getResources().getDimension(R.dimen.x30), 0);
...
//        mhsv_row.setBackgroundColor(context.getResources().getColor(R.color.black));
        ...
        // 发一个消息出去。当布局渲染完成之后会执行消息内容，此时 pl_root

        pl_root.post(new Runnable() {
            @Override
            public void run() {
                LogUtils.logd(TAG + "childAt是否是为空----" + lv_content);
                LogUtils.logd(TAG + "childAt是否是为空----" + (lv_content == null));
//                RelativeLayout childAt = (RelativeLayout) lv_content.getChildAt(0);
//                LogUtils.logd(TAG + "childAt是否是为空----" + (childAt == null));
                RelativeLayout childAt = (RelativeLayout) lv_content.getLayoutManager().findViewByPosition(0);
                if (childAt == null) {
                    childAt = (RelativeLayout) lv_content.getChildAt(0);
                }
...
//            int newLength = contentAdapter.getItemCount();
...
//        int rowCount = ll_contentItem.getChildCount();
        // 直接返回   传入的 横向表头
        List<String> rowDataList1 = rowDataList;
...
//            View contentItem = ll_contentItem.getChildAt(i);// 获得item的item，以便获取宽度
            ...
            rowItem.setGravity(isShouldBottomTitle ? Gravity.RIGHT | Gravity.CENTER_VERTICAL : Gravity.CENTER);
//            rowItem.setPadding(0,0, (int) context.getResources().getDimension(R.dimen.x20),0);
...
//        lv_column.addItemDecoration(columnDivider);
     ...
     */
    public AbstractPanelListAdapter(Context context, PanelListLayout pl_root, RecyclerView lv_content) {
